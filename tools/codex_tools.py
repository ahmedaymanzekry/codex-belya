...
class CodexTaskResult(TypedDict, total=False):
    """Structured result for Codex CLI interactions."""
...
class CodexTaskToolsMixin:
    ...
    @function_tool
    async def send_task_to_Codex(self, task_prompt: str, run_ctx: RunContext) -> CodexTaskResult:
        ...
        try:
            ...
            return {
                "output": None,
                "raw_result": None,
                "error": self._handle_tool_error("sending the task to Codex", error),
            }

    async def _a_long_running_task(self, task_prompt: str) -> CodexTaskResult:
